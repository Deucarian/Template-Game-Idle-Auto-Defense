...
- `Attacks` contains four generic attack recipes: Pulse Cannon, Shard Launcher, Arc Burst, and Homing Pulse.
- `Weapons` contains four tower weapon definitions paired with those attacks.
- `Waves` contains four spawn profiles: Opening, Pressure, Surge, and Final.
- `Upgrades` contains four run upgrades: Damage, Fire Rate, Range, and Projectile Speed.
...
The setup wizard copies these files into product-owned content folders, creates
fresh GUIDs, and rewrites the generated scene to reference the copied assets.
