...
Replaying the tutorial from a pause or settings menu must return to that paused menu. Verify both actual frame simulation and visible overlay state, then explicit resume and restart; completing the first-run tutorial still starts gameplay, and menu-only help does not start a run.
